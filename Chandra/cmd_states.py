...
def update_states_db(states, db, h5):
    ...
    if len(db_states) > 0:
        ...
        _delete_cmd_states(db_states['datestart'][i_diff], db, h5)

    else:
        # No cmd_states in database so just insert all new states
        i_diff = 0

    _insert_cmd_states(states, i_diff, db, h5)


def _delete_cmd_states(datestart, db, h5):
...
def _insert_cmd_states(states, i_diff, db, h5):
    """Insert new ``states[idiff:]`` into ``db`` and ``h5d``.
    """
    # As for _delete_cmd_states do the h5d insert first so if something
...
def _make_hdf5_cmd_states(db, h5):
...
def update_cmd_states():
    ...
    state0 = get_state0(date=opt.datestart, db=db)
    ...
    cmds = get_cmds(datestart, db=db, update_db=True,
